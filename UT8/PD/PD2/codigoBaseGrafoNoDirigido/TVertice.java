...
public class TVertice<T> implements IVertice {
    ...
    public TCaminos todosLosCaminos(Comparable etVertDest, TCamino caminoPrevio, TCaminos todosLosCaminos) {
        this.setVisitado(true);
        TCamino copiaCamino = new TCamino(caminoPrevio); // Asume que tienes un constructor copia
        copiaCamino.agregarAdyacencia(new TAdyacencia(0.0, this)); // O ajusta el costo según tu lógica

        if (this.getEtiqueta().equals(etVertDest)) {
            todosLosCaminos.agregarCamino(copiaCamino);
        } else {
            for (TAdyacencia ady : this.adyacentes) {
                TVertice destino = ady.getDestino();
                if (!destino.getVisitado()) {
        ...
        this.setVisitado(false); // Importante para permitir caminos alternativos
 ...
 public boolean tieneAdyacente(Comparable etiquetaDestino) {
